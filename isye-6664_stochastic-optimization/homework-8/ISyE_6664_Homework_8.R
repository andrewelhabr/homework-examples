...
Bellman <- function(s, a, R, P, V_current, alpha) {
  B[s, a] <- R[s, a] + sum(P[a, , s] * V_current)
  B[s, a]
}
...
Value_Iteration <-
  function(S,
    ...
    while (i <= max_iter & threshold) {
      ...
      # Checking if span semi-norm threshold has been violated yet.
      if (max(abs(V[, i] - V_current)) - min(abs(V[, i] - V_current)) > epsilon)
        {
        threshold <- TRUE
      } else {
        threshold <- FALSE
