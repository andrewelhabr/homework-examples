...
#mean_completion_times_95CI_LB <- rep(0,length(trials))
#mean_completion_times_95CI_UB <- rep(0,length(trials))
...
for (k in 1:length(trials)){
  ...
  #mean_completion_times_95CI_LB[k] <- mean_completion_times[k]-qt(0.975,trials[k]-1)*mean_completion_times_sd[k]/sqrt(trials[k])
  #mean_completion_times_95CI_UB[k] <- mean_completion_times[k]+qt(0.975,trials[k]-1)*mean_completion_times_sd[k]/sqrt(trials[k])
  ...
  #durations_df <- as.data.frame(durations)
  #print(ggplot(durations_df, aes(x=durations_df[,9])) + 
  #  geom_histogram(binwidth=2,color="black",fill="black") +
  #  geom_vline(aes(xintercept=mean(durations_df[,9])),color="blue",size=1)+
  #  labs(title=paste("Project Duration Histogram for",trials[k],"Trials"),x="Time (min)", y = "Count"))
}

mean_completion_times
#mean_completion_times_95CI_LB
#mean_completion_times_95CI_UB
#mean_completion_times_CI_length = mean_completion_times_95CI_UB - mean_completion_times_95CI_LB
#mean_completion_times_CI_length
mean_completion_times_95CI_HL
percentile_estimates
percentile_estimates_95CI_UB
percentile_estimates_95CI_LB
#percentile_CI_length = percentile_estimates_95CI_UB - percentile_estimates_95CI_LB
#percentile_CI_length
...
suppressWarnings(ggplot(durations_to_plot, aes(x=durations)) + 
  ...
  #geom_vline(aes(xintercept=mean(durations)),color="blue",size=1)+
...
#Y_bar_95CI_LB
Y_bar_95CI_HL
#Y_bar_95CI_length <- Y_bar_95CI_UB - Y_bar_95CI_LB
#Y_bar_95CI_length
